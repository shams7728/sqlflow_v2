...
const MobileMenu: React.FC<MobileMenuProps> = ({ lessons, isOpen, onClose }) => {
  const location = useLocation();
  const navigate = useNavigate();
  const [expandedSections, setExpandedSections] = useState<string[]>(['Getting Started']);
  const { progress, stats, achievements } = useProgressStore();


  // Close menu when route changes (but not on initial load)
  useEffect(() => {
    if (isOpen) {
      onClose();
    }
  }, [location.pathname]); // Removed onClose from dependencies to prevent unnecessary calls
  ...
  return (
    <>
      ...
      {/* Mobile Sidebar */}
      <div className="fixed inset-y-0 left-0 w-80 max-w-[85vw] bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 shadow-2xl z-50 lg:hidden transform transition-transform duration-300 ease-in-out"
           style={{ 
        ...
        <div className="flex flex-col h-full">
            ...
            <div className="bg-gradient-to-r from-primary-50 to-primary-100 dark:from-primary-900/20 dark:to-primary-800/20 rounded-xl p-4">
              ...
              <div className="w-full bg-primary-200 dark:bg-primary-800 rounded-full h-2">
                <div 
                  ...
                  style={{ 
                    width: stats ? `${(stats.completedLessons / stats.totalLessons) * 100}%` : '0%' 
                  }}
                ></div>
              </div>
              <div className="flex items-center justify-between mt-2 text-xs text-primary-600 dark:text-primary-400">
                <span>{stats ? `${stats.completedLessons} of ${stats.totalLessons}` : '0 of 0'} lessons</span>
          ...
          <div className="px-4 py-4 border-b border-gray-200/50 dark:border-gray-700/50">
            <nav className="space-y-1">
              {/* Main Navigation */}
              {navigationItems.filter(item => ['dashboard', 'lessons', 'practice', 'achievements', 'progress'].includes(item.id)).map((item) => {
                    ...
                    {isExpanded && (
                      <div className="space-y-1 ml-4 animate-slide-down">
                        {categoryLessons.slice(0, 8).map((lesson, index) => {
                          ...
                          const isCurrent = isInProgress || (!isCompleted && index === 0);
